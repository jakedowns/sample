...
const is_registered = $("#thisbidderTB").val() !== "0" && $("#streamPaddleNo").html();
...
/* TEMP */
// no need for domready, we inject this script after domready
SetAutoTick();

if(!is_registered){
    $('.streaming--paddleno').html('&nbsp;<span id="streamPaddleNo"></span>');
}
/* END TEMP */
    ...
    if (json && json.cb - 0 > 0) {
        ...
        if (json.hb != $("#thisbidderTB").val()) {
                ...
                if (json.hub) {
                    if ($("#myBid").val() != "" && $("#myBid").val() - 0 > json.hb - 0) {
                        ...
                        if (json.lbidct > 0) {
                            ...
                            $("#state_outbid > div.streaming--slider > p > span:last").html(
            ...
            if (json.cb - 0 > 0) {
                ...
                $("#bidbtnwinning").attr(
                    "value",
                    "Bidding " + formatter.format(json.cb)
                );
                $("#state_winning > div.streaming--slider > p > span:last").html(
                    "Bidding " + json.cb
                );
            }
        }
        ...
        $("#LiveAuctionCurrentLotImage").attr(
            "src",
            currentLot.image.replace("/200", "/1200")
        );
        $("#estimatespan").html(currentLot.estimate);
        $("#titlespan").html(
            "<strong>" + currentLot.artist + "</strong><br>" + currentLot.title
        );
        ...
        if (json.v != auctionState.v) {
            ...
            if (
                prevLot != json.ln &&
                (json.ln != "" && json.ln != "undefined" && prevLot != "")
            ) {
...
function getprevLotResult(pLot) {
    var cDate = new Date();
    var url =
        "AjaxLibSA.aspx?command=GetLotResult&lot=" +
        pLot +
        "&sess=" +
        $("#auctionsessionTB").val() +
        "&id=" +
        cDate.toTimeString();
    html = jQuery.ajax({
        url: url,
...
function appendFakeLots() {
    /* Appends fake lots */
    var jquery__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/ __webpack_require__.n(
    ...
    for (let i = 500; i < 504; i++) {
        ...
        let itemDiv =
            ...
            "</p>" +
            "</div>";
        jquery__WEBPACK_IMPORTED_MODULE_1___default()(".upcoming .carousel").append(
            itemDiv
        );
...
function applyUpdate(newState) {
    ...
    if (newState.s == STATUS_FINAL_CALL) {
        if ($("#state_waiting").hasClass("show")) {
            ...
            $(".pending > div.streaming--slider > p > span:last").html(
                "Bid Submitted for: " + formatter.format(newState.nb)
            );
            $(".pending > :button ").val(
                "Bid Submitted for: " + formatter.format(newState.nb)
            );
...
function GetUCLHTML(lot) {
    return (
        "<div id='UCL-" +
        lot.id +
        "' class='LiveAuctionUCL'>" +
        "<div class='LiveAuctionUCLLotNumber'>" +
        "<span class='LiveAuctionUCLLotNumberLbl'>Lot #</span>" +
        "<span class='LiveAuctionUCLLotNumberData'>" +
        lot.lotNumber +
        "</span>" +
        "<span class='LiveAuctionUCLLotNumberDivider'>-</span>" +
        "</div>" +
        "<div class='LiveAuctionUCLTitle'>" +
        lot.title +
        "</div>" +
        "<div class='LiveAuctionUCLImgDiv'><a href='" +
        lot.url +
        "' target='_blank'><img class='LiveAuctionUCLImg' src='" +
        lot.image +
        "' /></a></div>" +
        "<div class='LiveAuctionUCLCurrentBidDiv'><span class='LiveAuctionUCLCurrentBidLbl'>Current Bid: </span><span class='LiveAuctionUCLCurrentBidData'>" +
        formatter.format(lot.currentBid) +
        "</span></div>" +
        "<div class='LiveAuctionUCLNumBidsDiv'>" +
        "<span class='LiveAuctionUCLNumBidsData'>" +
        lot.numBids +
        "</span>" +
        ...
        (thisBidder.id != -1
            ...
            formatter.format(lot.nextBid) +
            "' onclick='placeBidUCL(" +
            lot.id +
            ", " +
            lot.nextBid +
            ");' />"
            : "<input type='button' value='Login to Bid' onclick='window.location=\"Login.aspx?back=LiveAuction2.aspx?sessionid=" +
            auctionSessionId +
            "\"' />") +
        "</div>" +
        (thisBidder.id != -1 && lot.winner == thisBidder.id
            ? "<div class='LiveAuctionUCLHighBidderDiv'>You are the high bidder</div>"
            : lot.hb
                ? "<div class='LiveAuctionUCLOutBidDiv'>You have been outbid</div>"
                : "") +
        "</div>"
    );
}
...
function openLot(lot, reopen, hasUserBid) {
    if (lot != null && (currentLot == null || currentLot.id != lot.id)) {
        ...
        $("#LiveAuctionCurrentLotImage").attr(
            "src",
            lot.image.replace("/200", "/1200")
        );
        $("#LiveAuctionCurrentLotImageLg").attr(
            "href",
            lot.image.replace("mid", "med")
        );
        if (currentLot.isPremium == 1) {
            $("#LiveAuctionCurrentLotPremiumImage").attr(
                "src",
                "/images/PremierLotGFA.gif"
            );
        } else {
        ...
        if (canBid()) {
            ...
            setHeaderText(
                "This lot is only available to Premier Bidders.  Please wait for the next one."
            );
            $("#LiveAuctionLotInfoPanelHeader").html("Bids are not allowed");
            $("#LiveAuctionLotInfoPanelHeader")
                .removeClass()
                .addClass("BidsNotAllowed");
            setBidButtonEnabled(false);
        }

        if (reopen) {
            addMessageListItem(
                formatLotAsHistoryLink(lot) + " has been reopened",
                "lot_repoened"
            );
        }
    }
...
function setBidValue(nextBid) {
    if (nextBid == null) {
        ...
        //   $('#LiveAuctionBidInfoAskingBidValue').html(formatter.format(nextBid));
        //   $('#LiveAuctionBidInfoPlaceBidBtn').val('Bid ' + formatter.format(nextBid));
        nextBidValue = nextBid;
    }
}

function placeBid(form) {
    console.log('placeBid', form);
    thisBidder.id = $("#thisbidderTB").val() - 0;
    thisBidder.isPremium = "0";
    ...
    if (res.error == null) {
        ...
        if (json) {
            ...
            if (json.lotNumber == currentLot.lotNumber) {
                ...
                //$('#LiveAuctionBidInfoCurrentBidValue').html(formatter.format(json.currentBid));
            }
        } else {
...
function updateBids(newState) {
    if (
        newState.ln == auctionState.ln &&
        newState.bh.length < auctionState.bh.length
    ) {
        deleteBidCB(newState);
    }

    if (newState.cb !== -1) {
        $("#LiveAuctionBidInfoCurrentBidValue").html(formatter.format(newState.cb));
    }
...
function StartAuction() {
    document.getElementById(
        "LiveAuctionBidInfoAdminStartAuctionBtn"
    ).disabled = true;
...
function addLotToHistory(lot, lastLotPassed) {
    ...
    var msgClass = "ended";
    if (lastLotPassed) {
        message += formatLotAsHistoryLink(lot) + " passed";
        msgClass = "lot_passed";
    } else if (thisBidder.id != -1 && lot.winner == $("#thisbidderTB").val()) {
        message +=
            "You won " + formatLotAsHistoryLink(lot).toLowerCase() + " for " + formatter.format(lot.currentBid);
        addMessageListItem(message, "lot_won");
        return;
    } else if (lot.currentBid == 0 || lot.hasLiveBids == 0) {
        if (lot.hasStaticBids) {
            message += formatLotAsHistoryLink(lot) + " sold for " + formatter.format(lot.currentBid);
            msgClass = "lot_sold";
        } else {
            message += formatLotAsHistoryLink(lot) + " passed";
            msgClass = "lot_passed";
...
function formatLotAsHistoryLink(lot) {
    return (
        '<span class="LiveAuctionMessageSpan">Lot ' + lot.lotNumber + "</span>"
    );
}

function addMessageListItem(message, cssClass) {
    $(
        '<li class="LiveAuctionMessageListItem ' +
        cssClass +
        '">' +
        message +
        "</li>"
        ...
        .hide()
        .prependTo("#LiveAuctionMessageList")
        .slideDown("slow");
}
...
function updateBidHistory(newState) {
    ...
    for (var len = messages.length - 1, i = len; i >= 0; i--) {
        ...
        if (msg && msg.i > lastMessage) {
            // use to have an Auctioneer: prefix here
            var message = toTitleCase(msg.m);
            if(message.toLowerCase().includes("thank you for your bids")){
                message = "Thank You For Bidding";
            }
            addMessageListItem(message, "message");
            ...
            if (mymsg.includes("last call")) {
                if ($("#state_winning").hasClass("show")) {
                    ...
                    $("#bidlastcall").attr(
                        "value",
                        "Bid " + formatter.format(newState.nb)
                    );
                    $("#state_lastcall > div.streaming--slider > p > span:last").html(
                        "Bid " + formatter.format(newState.nb)
                    );
                }
            }
...
function addBidToHistory(bidder, bid, bidId, hasBids) {
    ...
    var msgClass = "";
    if (bidder == thisBidder.id) {
        newItem += "You are the high bidder at ";
        msgClass = "bid_winning";
    } else if (bidder == floorBidder) {
        newItem += "Floor bids ";
        msgClass = "bid_floor";
    } else {
        newItem += "Internet bids ";
        msgClass = "bid_internet";
    }

    newItem += formatter.format(bid);
    topBid = new Object();
    topBid.id = bidId;
    topBid.bid = bid;
...
function deleteBidCB(newState) {
    if (newState.bh.length == 0) {
        ...
        if (topBid.bidder == thisBidder.id) {
            ...
            $("#bidbtnwinning").attr(
                "value",
                "You are Bidding " + formatter.format(newState.nb)
            );
        } else if (newState.hub) {
            // $('#LiveAuctionLotInfoPanelHeader').html('You have been outbid');
            if (is_registered) {
    ...
    var str = "The clerk has deleted a bid;";
    str += " the next bid is " + formatter.format(newState.nb);
    addMessageListItem(str, "bid_deleted");
}

// un-used, use formatter.format()
// function formatCurrency(total) {
//     return "$" + parseInt(total).toLocaleString('en-US', {minimumFractionDigits: 0, maximumFractionDigits: 0});
// }
            ...
            if (typeof lotHash[newState.ub[i].ln] != "undefined") {
                ...
                /*
                        if ($("#UCL-" + lotHash[newState.ub[i].ln].id).exists() && thisBidder.id != -1) {
                            $("#UCL-" + lotHash[newState.ub[i].ln].id + " input").val("Bid " + formatter.format(newState.ub[i].nb))
