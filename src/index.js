...
window.user_is_registered = $("#thisbidderTB").val() !== "0" && $("#streamPaddleNo").html();
...
function addLotToHistory(lot, lastLotPassed) {
  ...
  if (lastLotPassed) {
    ...
    message +=
      "You Won " + formatLotAsHistoryLink(lot) + " for $" + lot.currentBid;
...
function resetButtons() {
  $(
    ".streaming--bid:not(#state_logged_out):not(#state_not_registered)"
  ).removeClass("show");
  $(".pending > p.streaming--text:first").removeClass("shadow");
  $(".pending > p.streaming--text:last").addClass("shadow");
  $(".pending > :button ").prop("disabled", false);
